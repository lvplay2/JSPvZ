(function () {
    ...
    do {
        e.push(q.substr(s, 1));
        r = Number(q.substr(s + 1, 1));
        r > c && (c = r);
        n.push(r);
        g.push(j[q.substr(s + 2, 2)]);
        s += 4;
    } while (s < k);
    oS.Init(
        {
            PName: g,
            ZName: [oImp, oZombie, oConeheadZombie, oPoleVaultingZombie, oBucketheadZombie, oScreenDoorZombie, oFootballZombie, oDancingZombie, oBackupDancer],
            ...
            RiddleAutoGrow: function () {
                var t = g.length;
                while (t--) {
                    CustomSpecial(g[t], e[t], n[t], 1);
                }
                t = 6;
                while (--t) {
                    CustomSpecial(oBrains, t, 0);
                }
                NewImg("iStripe", "images/interface/Stripe.png", "left:" + (GetX1X2(c + 1)[0] - 11) + "px;top:65px", EDAll);
            },
            ...
            LoadAccess: function (i) {
                ...
                NewEle("JSPVZAjax", "script", 0, { src: $User.Server.DataURL + "asp/GetImZombieCreateGameVNumAdd.asp?id=" + o, type: "text/javascript" }, document.body);
                ...
                oS.ScrollScreen = function () {
                    ...
                    ClearChild($("dButton1"), $("dButton2"), $("dButton3"));
                    (function () {
                ...
                (function () {
                    NewEle(
                        "dPJ",
                        "div",
                        "position:absolute;left:230px;top:0;width:460px;height:50px;z-index:19",
                        {
                            innerHTML:
                    ...
                    var t = function (w) {
                            ...
                            ClearChild($("JSPVZAjax"));
                            NewEle("JSPVZAjax", "script", 0, { src: $User.Server.DataURL + "asp/ImZombieCreateGameVote.asp?id=" + o + "&Vote=" + w, type: "text/javascript" }, document.body);
                            var y = $("dPJ"),
                                z = y.childNodes,
                                x = z.length,
                                A;
                ...
                ($User.Visitor.UserName == p || $User.Visitor.UserAuthority == 255) &&
                    NewEle(
                        "dButton3",
                        "button",
                            ...
                            onclick: function () {
                                if (confirm("你确定要删除这个关卡？一旦删除无法恢复！")) {
                                    ...
                                    if ((t = prompt("输入删除的理由\n未输入则默认为空", "")) != null) {
                                        ...
                                        ClearChild($("JSPVZAjax"));
                                        NewEle("JSPVZAjax", "script", 0, { src: $User.Server.DataURL + "asp/ImZombieCreateGameDel.asp?R=" + escape(t) + "&GID=" + o, type: "text/javascript" }, document.body);
                                    }
                                }
                            },
                        },
            ...
            FlagToEnd: function () {
                ...
                i.isAuthorWebsite &&
                    ...
                    NewEle(
                        ...
                        "script",
                        0,
                        { src: $User.Server.DataURL + "asp/GetImZombieCreateGameFNumAdd.asp?id=" + o + "&N=" + ArPCard.SelNum + "&S=" + oS.SunNum + "&T=" + (oSym.Now - oS.StartTime), type: "text/javascript" },
                        document.body
                    ));
        ...
        },
        {
            AutoSelectCard: function () {
                var v = oS.ArCard,
                    ...
                    t = v.length - 1;
                while (++u < t) {
                    SelectCard(v[u].prototype.EName);
                }
            },
        }
    );
